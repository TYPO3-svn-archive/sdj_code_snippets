 ...
 * @copyright (C) 2004 - 2007 Nigel McNie, (C) 2007 - 2008 Benny Baumann
 */
class tx_sdjcodesnippets_geshi {
    /**#@+
     * @access private
     */
    /**
            ...
            if ($highlight_part) {
                ...
                for ($i = 0; $i < $length; ++$i) {
                    ...
                    if ($string_started && ($i != $next_comment_regexp_pos)) {
                        ...
                        $string = "<span$string_attributes>" . tx_sdjcodesnippets_geshi::hsc($char);
                        $start = $i + $char_len;
                        $string_open = true;
...
//        $result = substr($result, 1, -1);
        ...
        if($this->language == 'typoscript') {
        	$a_lines = t3lib_div::trimExplode("\n",$endresult);
        	$i_lines = count($a_lines);
        	for($i = 0; $i < $i_lines; $i++) {
        		$a_lineParts = explode('<span class="sy0">=</span>',$a_lines[$i],2);
        		if(count($a_lineParts) == 2) {
        			if(strstr($a_lineParts[1],'class="kw4"')) continue;
        			$a_lineParts[1] = '<span class="val">'.strip_tags($a_lineParts[1]).'</span></span></li>';
        		}
        		$a_lines[$i] = implode('<span class="sy0">=</span>',$a_lineParts);
        	}
        	$endresult = implode("\n",$a_lines);
        }
        return $endresult;
        ...
        if ($this->line_numbers != GESHI_NO_LINE_NUMBERS && $this->header_type != GESHI_HEADER_PRE_TABLE) {
            ...
            for ($i = 0, $n = count($code); $i < $n;) {
                ...
                if ($this->header_type == GESHI_HEADER_PRE_VALID) {
                    ...
                    $start = ''; //"<div$def_attr>";
                    $end = ''; //</div>';
                    $start = "<span$def_attr>"; //"<div$def_attr>";
                    $end = '</span>'; //</div>';
                }
    ...
    function header() {
         ...
         */
        $attributes = ' class="code ' . $this->_genCSSName($this->language);
...
if (defined('TYPO3_MODE') && $TYPO3_CONF_VARS[TYPO3_MODE]['XCLASS']['ext/sdj_code_snippets/class.tx_sdjcodesnippets_geshi.php'])	{
	include_once($TYPO3_CONF_VARS[TYPO3_MODE]['XCLASS']['ext/sdj_code_snippets/class.tx_sdjcodesnippets_geshi.php']);
}
?>